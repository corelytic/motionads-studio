...
function buildStandaloneHtml(project) {
  const payload = {
    ...
    offer: project.offer,
    cta: project.cta,
    productStudio: project.productStudio || null,
  ...
  const studio = {
    ...
    shadowIntensity: Number(project.productStudio?.shadowIntensity || 48),
    glowIntensity: Number(project.productStudio?.glowIntensity || 32),
  ...
  const studioClass = [studio.heroMode ? "hero-boost" : "", studio.cinematicMode ? "cinematic-boost" : "", ...studio.effects.map((item) => `effect-${item}`)].filter(Boolean).join(" ");
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .heat-layer{inset:auto 0 -8% 0;height:44%;opacity:0;background:radial-gradient(circle at 50% 100%,rgba(255,171,82,.22),transparent 46%),repeating-linear-gradient(90deg,rgba(255,255,255,.03) 0 2px,transparent 2px 18px);filter:blur(calc(8px + (var(--overlay-intensity,.62) * 8px)));animation:heatShimmer 4.4s ease-in-out infinite}
    ...
    .copy{display:grid;gap:14px;align-content:end;animation:copyParallax 3.8s ease-in-out infinite;backdrop-filter:blur(calc(var(--studio-blur,0px) * .12));max-width:calc(100% - 10px)}
    ...
    .asset-card{
      ...
      transform:translate(var(--product-x,0px),var(--product-y,0px)) rotate(var(--product-rotation,0deg)) scale(var(--product-zoom,1));
      box-shadow:
        0 calc(18px + (var(--product-shadow,32) * .12px)) calc(46px + (var(--product-shadow,32) * .35px)) rgba(4,10,20,.32),
        0 0 calc(8px + (var(--product-glow,24) * .28px)) rgba(255,255,255,.14);
    }
    .asset-card img{display:block;max-width:min(240px,32vw);max-height:min(240px,32vw);border-radius:24px;object-fit:cover;filter:brightness(var(--product-brightness,100%)) contrast(var(--product-contrast,100%)) saturate(var(--product-saturation,100%))}
    .asset-fallback{
    ...
    .hero-boost .asset-card,.cinematic-boost .asset-card{box-shadow:0 32px 70px rgba(4,10,20,.36),0 0 34px rgba(255,255,255,.12)}
    .cinematic-boost{box-shadow:inset 0 1px 0 rgba(255,255,255,.08),inset 0 -90px 160px rgba(0,0,0,.34)}
    .effect-sale-energy .offer-burst{background:linear-gradient(135deg,rgba(255,104,104,.2),rgba(255,183,77,.22))}
    .effect-luxury-glow .beam{opacity:.28;animation-duration:9.2s}
    .effect-food-heat .heat-layer{opacity:.9}
    .effect-cinematic-lighting .streak{opacity:.76}
...
<body>
  <main class="shell">
    <section class="hero">
      ...
      <section class="stage-shell" aria-label="Animated campaign preview">
        ...
        <section class="stage industry-${industryKey} style-${styleKey} ${studioClass}" id="stage" style="--product-x:${studio.x}px; --product-y:${studio.y}px; --product-zoom:${studio.zoom}; --product-rotation:${studio.rotation}deg; --product-brightness:${studio.brightness}%; --product-contrast:${studio.contrast}%; --product-saturation:${studio.saturation}%; --product-shadow:${studio.shadowIntensity}; --product-glow:${studio.glowIntensity}; --studio-blur:${Number(project.productStudio?.blurBackground || 0)}px; --overlay-intensity:${studio.overlayIntensity / 100};">
          <div class="noise"></div>
  ...
  <script>
    ...
    function buildScene(scene, index) {
      ...
      const imageMarkup = (typeof scene.image === "string" && scene.image.trim())
        ? '<div class="asset-card"><img alt="Campaign asset" src="' + esc(scene.image) + '"></div>'
        : '<div class="asset-fallback">' + esc(project.brandKit.logoText || (project.brandKit.brandName || "").slice(0, 2) || "CM") + '</div>';
      wrapper.innerHTML = '<div class="copy">' +
        '<span class="kicker">' + esc(${JSON.stringify((project.industry || "").toUpperCase())}) + ' | ' + esc(project.platform) + '</span>' +
        '<h3 class="headline">' + esc(scene.headline) + '</h3>' +
        '<p class="subheadline">' + esc(scene.subheadline || scene.body || "Motion scene") + '</p>' +
        '<div class="cta-row"><button class="cta">' + esc(scene.cta) + '</button><div class="tags"><span class="tag">' + esc(animationGlyph(scene.animationType)) + '</span><span class="tag">' + esc(scene.duration.toFixed(1) + 's') + '</span></div></div>' +
