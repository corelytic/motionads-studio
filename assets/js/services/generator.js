...
function createShapeElement(sceneId, layerName, shapeKind, fill, x, y, width, height, opacity = 1) {
  return { id: uid("el"), sceneId, layerName, type: "shape", shapeKind, fill, x, y, width, height, rotation: 0, opacity, hidden: false, locked: false, start: 0, end: 1 };
}
...
function sceneBlueprint(type, config, recipe) {
  ...
  const sceneMap = {
    ...
    cta: {
      name: "CTA Scene",
      headline: config.cta.toLowerCase().includes("now") ? titleCase(config.cta) : `${config.cta} now`,
